	...
	$.extend({ tablesorterPager: new function() {
		...
		updatePageDisplay = function(table, p, completed) {
			...
			if ( Math.min( p.totalPages, p.filteredPages ) >= 0 ) {
				...
				//alert('total : '+p.totalRows+' > '+p.totalPages+' > '+p.startRow+' > '+p.endRow);
		...
		parsePageNumber = function( p ) {
			...
			if (p.page >= 0 && p.pageInit != undefined) {
				p.page = p.pageInit;
				p.pageInit = undefined;
		...
		$this.updatePageDisplay = function(table, page) {
			alert(page);
		};
			...
			return this.each(function() {
				...
				$t
					...
					.bind('updateComplete' + namespace, function(e, table, triggered) {
						var updateParams = $(table).data('update_params');
						$(table).removeData('update_params');
						
						var set_page = false;
						
						if (updateParams) {
							if (updateParams['page']) {
								var page = intval(updateParams['page']);
								if (page < 0) page = 0;
								//alert(p.page+' > '+page);
								//if (p.page != page) {
									p.pageInit = page;
					...
					.bind('updateTotal', function(e, total) {
						e.stopPropagation();
						p.totalRows = total;
						updatePageDisplay(table, p, false);
					})
