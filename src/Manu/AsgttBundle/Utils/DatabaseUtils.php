...
class DatabaseUtils {
	...
	public static function quote($conn, $value) {
		if ($value === null) return 'null';
		
		if (!$conn) {
			$doctrine = ManagersManager::getManager()->getContainer()->get('doctrine');
	...
	public static function getFullSQL($query)
	{
		...
		for($i=0;$i<strlen($sql);$i++){
			if($sql[$i]=='?'){
				...
				//error_log('TEST PARAM : '.$nameParam);
				...
				elseif(is_array($paramsArr[$nameParam])){
					...
					} else if ($paramsArr[$nameParam] instanceof ClassMetadata) {
						$metadata = $paramsArr[$nameParam];
						if (false) $metadata = new ClassMetadata();
						$fullSql.= "'".$metadata->discriminatorValue."'";
					} else {
	...
	protected static function getListParamsByDql($dql)
	{
